...
describe BetterBusinessTime::Days do
  describe '.between' do
    ...
    it 'returns business days between two dates' do
      ...
      expect(described_method(d('Fri Nov 4'), d('Mon Nov 7'))).to eq(1)
    ...
    it 'excludes holidays' do
      ...
      expect(described_method(d('Tue Nov 1'), d('Fri Nov 4'))).to eq(2)
    end
  end
  ...
  describe '.weekend_days_between' do
    ...
    describe 'when distance < 1 week' do
      it 'returns 0 between monday and friday' do
        ...
        expect(described_method(d('Sat Nov 5'), d('Thu Nov 3'))).to eq(0)
        expect(described_method(d('Wed Nov 2'), d('Thu Nov 3'))).to eq(0)
      end

      it 'returns 1 if one of the two falls on a weekend' do
        expect(described_method(d('Sun Oct 30'), d('Fri Nov 4'))).to eq(1)
        expect(described_method(d('Sun Oct 30'), d('Mon Oct 31'))).to eq(1)
        expect(described_method(d('Mon Oct 31'), d('Sat Nov 5'))).to eq(1)
        expect(described_method(d('Fri Nov 4'), d('Sat Nov 5'))).to eq(1)
      end

      it 'returns 2 if both fall on a weekend' do
        expect(described_method(d('Sat Oct 29'), d('Sun Oct 30'))).to eq(2)
        expect(described_method(d('Sun Oct 30'), d('Sat Nov 5'))).to eq(2)
      end
    end
    ...
    describe 'when distance > 1 week' do
      ...
      it 'returns 5 if one of the two falls on a weekend' do
        expect(described_method(d('Sun Oct 16'), d('Fri Nov 4'))).to eq(5)
        expect(described_method(d('Sun Oct 16'), d('Mon Oct 31'))).to eq(5)
        expect(described_method(d('Mon Oct 17'), d('Sat Nov 5'))).to eq(5)
        expect(described_method(d('Fri Oct 21'), d('Sat Nov 5'))).to eq(5)
      end

      it 'returns 6 if both fall on a weekend' do
        expect(described_method(d('Sat Oct 15'), d('Sun Oct 30'))).to eq(6)
        expect(described_method(d('Sun Oct 16'), d('Sat Nov 5'))).to eq(6)
      end
    end
  end
end
